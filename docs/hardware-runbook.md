...
Leave hover trim disabled for autonomous runs until it is calibrated in a controlled manual test. If the Tello consistently drifts backward/right while holding hover, explicitly enable a small trim before launching. Positive `forward_back` nudges forward; negative `left_right` nudges left:
